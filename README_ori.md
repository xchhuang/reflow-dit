# ReDiT: combining Rectified flow and  DiT
...
**TL;DR**: This repo is built on top of [fast-DiT](https://github.com/chuanyangjin/fast-DiT), aiming to combing [Rectified flow](https://arxiv.org/abs/2209.03003) and [DiT](http://arxiv.org/abs/2212.09748) for few-step inference.


## My install
* local anaconda environment: dit


## Summary
* ImageNet256 dataset comes from Kaggle, with around 500,000 images, 11 GB .npy files.
* Training speed of fast-DiT reaches around 0.55 steps/sec using DiT-XL/2 with 4x NVIDIA Quadro RTX 8000. This took 960 GPU hours to from scratch.
* To rectify DiT, after first training, we generate a new dataset with 500,000 images using the trained DiT-XL/2 model with 50 steps of DDIM.
This new dataset is to train 1-rectified flow.
* Train 1-rectified flow, the quality of few-step inference seems improved. But overall quality with 50 steps seems not as good as the original DiT with DDIM.
* Repeat the last step to train 2-rectified flow.



<!-- ## Thoughts
* Training loss is not scaled consistently to visual quality. The visual quality improves a a lot during training but loss decreases from 0.38 to 0.36.
* It seems transformers are slow to train and there should be room for acceleration.  -->
...
* Our DiT generated samples, using DDIM with 250 steps, after 960 GPU hours of training from scratch:
![DiT samples](asset/sample_d12.png)


* Our final DiT generated samples, using DDIM with 50 steps:
